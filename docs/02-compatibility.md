...
| 设备驱动 | CH341（血压计）x86 驱动、VGA2USB（Epiphan）x86 驱动 |

> 32 位应用运行在 64 位 Win7 上时走 WoW64，仍是 32 位进程；驱动安装需匹配
> **操作系统位数**（x64 OS 装 x64 驱动），而不是应用位数。现有项目仅内置了
> `V2UInstaller64.msi`，**缺少 32 位驱动的安装包**，重构必须补齐。
...
| V10 | 驱动安装 | x86/x64 驱动按 OS 位数正确安装 |
